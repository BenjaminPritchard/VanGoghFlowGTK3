...
item_clicked_cb(GtkWidget *widget, gpointer data)
{
    ...
    loadURL(web_view, "https://www.youtube.com/embed/9TbLJI7ja4s?rel=0;&autoplay=1&mute=1");
}
...
void MakeTrayIcon()
{
    ...
    menu = gtk_menu_new();
    GtkWidget *item = gtk_check_menu_item_new_with_label("Visualization 1");
    g_signal_connect(item, "activate",
                     G_CALLBACK(item_clicked_cb), "1");
    gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);
    gtk_widget_show(item);

    item = gtk_radio_menu_item_new_with_label(NULL, "Visualization 2");
    g_signal_connect(item, "activate",
                     G_CALLBACK(item_clicked_cb), "2");
    gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);
    gtk_widget_show(item);

    item = gtk_check_menu_item_new_with_label("Visualization 3");
    g_signal_connect(item, "activate",
